...
import { matchesGlobLike, pathExists } from "../utils/pathSafety.js"

async function listDirectoryEntries(dirPath: string, recursive: boolean, showHidden: boolean): Promise<unknown[]> {
...
async function buildTree(root: string, depth: number): Promise<unknown> {
...
async function readFileText(filePath: string, encoding: BufferEncoding, startLine?: number, endLine?: number): Promise<string> {
...
export function registerFilesystemTools(server: McpServer, ctx: ToolContext): void {
  ...
  server.registerTool("fs_read_file", {
    ...
    const content = await readFileText(filePath, (encoding as BufferEncoding) ?? "utf8", startLine, endLine)
  ...
  server.registerTool("fs_write_file", {
    ...
    await fs.writeFile(filePath, content, { encoding: (encoding as BufferEncoding) ?? "utf8" })
  ...
  server.registerTool("fs_chown", {
    ...
    ctx.assertWritable()
    const filePath = ctx.resolvePath(target)
    const { stdout, stderr, exitCode } = await import("./terminal.js").then((mod) =>
      mod.execCommand(`chown ${user}${group ? `:${group}` : ""} ${JSON.stringify(filePath)}`, ctx.config.workspaceRoot, 30_000, {
        ...process.env,
      }),
    )
    if (exitCode !== 0) {
