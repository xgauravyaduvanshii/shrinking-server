  ...
  if (config.transport === "http") {
    const listener = http.createServer(async (req, res) => {
      try {
        ...
        // Fresh stateless transport per request matches the SDK example pattern.
        const transport = new StreamableHTTPServerTransport({
          sessionIdGenerator: undefined,
        })
        const server = createServer(ctx, nativeSocket, terminalWs, heartbeat)
        await server.connect(transport)
        await transport.handleRequest(req, res, body)
      } catch (error) {
        logger.error("HTTP transport request failed", { error: error instanceof Error ? error.message : String(error) })
        if (!res.headersSent) {
          res.statusCode = 500
