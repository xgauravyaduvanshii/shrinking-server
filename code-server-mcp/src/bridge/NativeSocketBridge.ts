...
import type { ClipboardArgs, NativeBridgeResult, OpenFileArgs, StatusResult } from "./types.js"
...
export class NativeSocketBridge extends EventEmitter {
  ...
  private discoverSocketPath(): string {
    ...
    try {
      const candidates = syncFs
        .readdirSync(tmpDir)
        .filter((entry: string) => entry.startsWith("vscode-ipc-") && entry.endsWith(".sock"))
        .map((entry: string) => {
  ...
  async openFile(args: OpenFileArgs): Promise<NativeBridgeResult> {
  ...
  async clipboard(args: ClipboardArgs): Promise<{ text?: string; result: "ok" | "error"; message?: string }> {
    if (args.action === "read") {
      return {
        result: "error",
  ...
  private async send(message: object): Promise<NativeBridgeResult> {
