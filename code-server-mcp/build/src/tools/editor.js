...
function replaceRange(content, args) {
...
async function ensureNativeSocket(bridge) {
    if (!bridge.isConnected()) {
        await bridge.connect().catch(() => undefined);
    }
    return bridge.isConnected();
}
function degraded(reason, fallback, extra = {}) {
...
export function registerEditorTools(server, ctx, nativeSocket) {
    ...
    server.registerTool("editor_open_file", {
        ...
        if (!available) {
            ...
            return degraded("Native socket not connected. Run bridge_reconnect or set VSCODE_IPC_HOOK_CLI.", "The file was still tracked by MCP and a CLI open was attempted.", { path: filePath, line: line ?? 1, column: column ?? 1 });
        }
    ...
    server.registerTool("editor_open_files_batch", {
        ...
        const available = await ensureNativeSocket(nativeSocket);
        if (!available) {
            return degraded("Native socket not connected. Run bridge_reconnect or set VSCODE_IPC_HOOK_CLI.", "Files were tracked locally but could not be opened in the visible window.", { paths: resolved });
        }
    ...
    server.registerTool("editor_get_status", {
        ...
        const available = await ensureNativeSocket(nativeSocket);
        if (!available) {
            return degraded("Native socket not connected. Run bridge_reconnect or set VSCODE_IPC_HOOK_CLI.", "Using MCP tracked editor state only.", { activeFile: ctx.editorState.activeFile ?? null, recentFiles: ctx.editorState.recentFiles });
        }
    ...
    server.registerTool("editor_run_command", {
        ...
        if (command === "workbench.action.reloadWindow") {
            return degraded("Reloading the code-server window is not exposed by the native CLI socket.", "Use the browser UI or add a deeper product integration inside code-server.", { command });
        }
        return degraded(`Unsupported editor command: ${command}`, "Use filesystem, LSP, or native open/status tools instead.", { command, args: args ?? [] });
    }));
    ...
    server.registerTool("clipboard_write", {
        ...
        const available = await ensureNativeSocket(nativeSocket);
        if (!available) {
            return degraded("Native socket not connected. Clipboard write is unavailable.", "Reconnect the bridge or use the browser session directly.");
        }
    ...
    server.registerTool("clipboard_read", {
        ...
        const available = await ensureNativeSocket(nativeSocket);
        if (!available) {
            return degraded("Native socket not connected. Clipboard read is unavailable.", "Reconnect the bridge or use the browser session directly.");
        }
