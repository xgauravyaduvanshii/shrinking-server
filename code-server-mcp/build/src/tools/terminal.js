...
export async function execCommand(command, cwd, timeout = 60_000, env = process.env) {
...
export function registerUITerminalTools(server, ctx, terminalWs) {
    const wrap = (toolName, handler) => async (args) => {
        try {
            ctx.guardTool(toolName, args);
            return await handler(args);
    ...
    const degraded = (reason, fallback, extra = {}) => successResult({
        success: false,
        degraded: true,
        reason,
        fallback,
