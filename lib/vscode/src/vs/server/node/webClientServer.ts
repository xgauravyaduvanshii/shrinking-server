...
const ISLANDS_THEME_NAME = 'Islands Dark';
const ISLANDS_PRESET_NAME = 'islands-dark';

type JsonObject = Record<string, unknown>;

function toCssRuleBody(value: JsonObject): string {
	return Object.entries(value).map(([property, propertyValue]) => {
		if (typeof propertyValue === 'string' || typeof propertyValue === 'number') {
			return `${property}:${propertyValue};`;
		}

		if (propertyValue && typeof propertyValue === 'object' && !Array.isArray(propertyValue)) {
			return `${property}{${toCssRuleBody(propertyValue as JsonObject)}}`;
		}
...
function stylesheetObjectToCss(stylesheet: JsonObject): string {
	return Object.entries(stylesheet).map(([selector, selectorValue]) => {
		if (!selectorValue || typeof selectorValue !== 'object' || Array.isArray(selectorValue)) {
			return '';
		}

		return `${selector}{${toCssRuleBody(selectorValue as JsonObject)}}`;
	}).join('\n');
}
...
export class WebClientServer {
	...
	private async _readUserSettings(): Promise<JsonObject | undefined> {
		try {
			const settingsPath = resolve(this._environmentService.userDataPath, ...USER_SETTINGS_PATH);
			return JSON.parse(await promises.readFile(settingsPath, 'utf8'));
		} catch {
			return undefined;
		}
	}

	private async _getIslandsInjectedCss(staticRoute: string): Promise<string> {
		const userSettings = await this._readUserSettings();
		if (!userSettings) {
			return '';
		}

		const hasIslandsTheme = userSettings['workbench.colorTheme'] === ISLANDS_THEME_NAME;
		const hasIslandsPreset = userSettings['custom-ui-style.preset'] === ISLANDS_PRESET_NAME;
		const stylesheet = userSettings['custom-ui-style.stylesheet'];
		if (!hasIslandsTheme && !hasIslandsPreset) {
			return '';
		}

		const stylesheetCss = stylesheet && typeof stylesheet === 'object' && !Array.isArray(stylesheet)
			? stylesheetObjectToCss(stylesheet as JsonObject)
			: '';
		...
		const islandsFontsBase = `${staticRoute}/extensions/islands-dark/fonts`;
		const presetFontsBase = `${staticRoute}/extensions/custom-ui-style/res/presets/${ISLANDS_PRESET_NAME}/fonts`;
		return `
<style id="shrinking-server-islands-style">
@font-face {
	font-family: 'Bear Sans UI';
	src: url('${islandsFontsBase}/BearSansUI-Regular.otf') format('opentype');
	font-style: normal;
	font-weight: 400;
	font-display: swap;
}
@font-face {
	...
	src: url('${islandsFontsBase}/BearSansUIHeading-Bold.otf') format('opentype');
	font-style: normal;
	font-weight: 700;
	font-display: swap;
}
@font-face {
	...
	src: url('${presetFontsBase}/IBMPlexMono-Regular.ttf') format('truetype');
	font-style: normal;
	font-weight: 400;
	font-display: swap;
}
${stylesheetCss}
	...
	private async _handleRoot(req: http.IncomingMessage, res: http.ServerResponse, parsedUrl: url.UrlWithParsedQuery): Promise<void> {
		...
		const values: { [key: string]: string } = {
			...
			WORKBENCH_BRANDED_CSS: await this._getIslandsInjectedCss(staticRoute),
			BASE: rootBase,
			VS_BASE: basePath,
		};
